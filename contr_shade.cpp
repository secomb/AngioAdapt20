	...
	int showscale, int lowcolor, int hatch, int plotcontour)
{
	...
	if (hatch > 0) {	//changed parameters here to cover more of page with hatching. Feb. 2017
		fprintf(ofp, "/diagonals1\n");
		fprintf(ofp, "{ newpath\n");
		fprintf(ofp, "-900 10 550\n");
		fprintf(ofp, "{ 0 moveto\n");
		fprintf(ofp, "900 900 rlineto } for\n");
		fprintf(ofp, "stroke } def\n");

		fprintf(ofp, "/diagonals2\n");
		fprintf(ofp, "{ newpath\n");
		fprintf(ofp, "-900 10 550\n");
		fprintf(ofp, "{ 900 moveto\n");
		fprintf(ofp, "900 -900 rlineto } for\n");
		fprintf(ofp, "stroke } def\n");
	...
	if (showscale > 0) {
		...
		fprintf(ofp, "%f mx %f my 10 sub m (Time = %6.1f) show\n", xmin, ymin, time1);
	}
	free_dvector(xv, 1, m);
	free_dvector(yv, 1, n);
