...
void greens(void)
{
	...
	extern float **cnode,**scos,**ax,**bcp;
...
//index tissue points
	for(i=1; i<=mxx; i++) for(j=1; j<=myy; j++) for(k=1; k<=mzz; k++){
		...
		if(nt > 0){
			...
			sourcefac[nt] = 1.;
			sourcefacGF[nt] = 1.;
			if ((k - 0.5) * 5 <= mzz) sourcefac[nt] = 0.;	//turn off oxygen consumption near bottom surface
			if ((mzz - k + 0.5) * 10 <= mzz) sourcefac[nt] = 0.;	//turn off oxygen consumption near top surface
			if ((k - 0.5) * 20 <= mzz) sourcefacGF[nt] = 0.;	//turn off GF production near bottom surface
			//sourcefac[nt] = 1. - exp(-5. * (k - 0.5) / mzz) - exp(-10. * (mzz - k + 0.5) / mzz);	//exponential taper top and bottom
			//sourcefacGF[nt] = 1. - exp(-20. * (k - 0.5) / mzz);	//exponential GF taper bottom only
			//if (5. * (k - 0.5) < mzz) sourcefac[nt] = 5. * (k - 0.5) / mzz;	//taper oxygen consumption near bottom surface
			//if (10. * (mzz - k + 0.5) < mzz) 
			//	sourcefac[nt] = 10. * (mzz - k + 0.5) / mzz;	//tapers oxygen consumption near top surface
		}
	}
...
//contribution ptt from tissue source strengths qt
			if(useGPU){
//GPU method NOT using successive updating of tissue source strengths: uses block updating. May 2016.
...
//non-GPU method using successive updating of tissue source strengths
			else{
				for(itp=1; itp<=nnt; itp++){
					ix = tisspoints[1][itp];
					iy = tisspoints[2][itp];
		...
		if(useGPU){
			...
			for(itp=1; itp<=nnt; itp++){
				...
				for(jtp=1; jtp<=nnt; jtp++){
					...
					iydiff = abs(iy - jy) + 1;
					izdiff = abs(iz - jz) + 1;
					for(isp=1; isp<=nsp; isp++) if(diffsolute[isp] == 1) g0facnew[isp] += dtt[ixdiff][iydiff][izdiff]/diff[isp]*mptiss[isp]*vol * sourcefac[itp];
				}
			}
		}
		for(isp=1; isp<=nsp; isp++) if(diffsolute[isp] == 1 && (g0method == 2 || permsolute[isp] == 0)){
			g0facnew[isp] = 1./(1. - g0facnew[isp]/nnt);
			dqsumdg0 = FMIN(dqtsumdg0[isp],0.)*g0facnew[isp];	//dqvsumdg0 is not implemented so don't use! June 2016 
...
//********************** end of main loop *****************************
	...
	if(qvsum[isp] != qvsum[isp])
		printf("*********\n");
	if(qtsum[isp] != qvsum[isp])
		printf("*********\n");
