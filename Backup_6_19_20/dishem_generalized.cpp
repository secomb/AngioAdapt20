...
//boundary nodes
	for(bnod=1; bnod<=nnodbc; bnod++){
		...
		if(nodout[inod] >= 1){
			...
			hd[iseg] = bchd[bnod];
			if(hd[iseg] > 1.F){
				printf("*** Warning: hd[seg] > 1 at boundary node\n");
			}
			isegk++;
		}
	}
			...
			if(nout == nodt) for(i=1; i<=nodt; i++)	hd[nodseg[i][inod]] = bchd[1];//This should not normally happen
				...
				if(nout == 1){
					...
					if(ibif < nout-1){
						...
						hd[seg2] = (1.-rbcrat)*hq/fabs(q[seg2]);	//last remaining outflow
						if(hd[seg2] > 1.F){
							printf("*** Warning: hd[seg2] > 1\n");
						}
					}
				}
			}
