...
void conductconvect()
{
	extern int nnodfl,nseg,slsegdiv,hexperiodic;
	extern int *nodtyp,*nodtyp,*nodfltyp,*segtyp,*lowflow,*ista,*iend;
	//extern float kp1, km1, kc1, ks1, L1, Qref1, pO2ref1, nmeta1, thresh1, req;//TWS2010,JPA2013
	extern float nmeta1, thresh1, req;//TWS2020
	...
	for(iseg=1; iseg<=nseg; iseg++)	if(segtyp[iseg] >= 3 && segtyp[iseg] <= 5){
		...
		metsig[iseg] = 3. * (1. - 1. / (1. + powf(GF / thresh1, nmeta1)));		//Hill-type GF-only effect. June 2020		
	}
	putrank();
	convectdown1();
	conductup1();
