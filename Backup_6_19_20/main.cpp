...
int max = 100, nseg, nnod, nsubseg, nt, nnodbc, niter, nnodfl, nsegfl, segfltot, imain, imainmax;
...
int seed, actnum, actnum_dim, sprnum, nsprout, actupdateflag, flag;
int dogreensflag;
...
float smax, new_diam, GFmax, nmeta1, thresh1;
float kp1, km1, kc1, ks1, ksnew, ranks1, L1, J01, tauref1, tauref2, Qref1, pO2ref1, extlength1in, extlength1out, timestep1;
float thresh, threshn, thresh50, ks, kgGF, kgVAF, kv, lambdatension, tensionrate, kmetdiam;
...
int main(int argc, char* argv[])
{
	...
	for (imain = 1; imain <= imainmax; imain++) {
		...
		if (dogreensflag) {
			...
			contourO2GF(imain);
			cmgui(imain);				//make files for cmgui. Feb. 2017
			for (iseg = 1; iseg <= nseg; iseg++) segvar[iseg] = iseg;
			for (inod = 1; inod <= nnod; inod++) nodvar[inod] = inod;
		...
		//write_network(imain);		//writes numbered network file in Current
		analyzenet(0);
		if (adaptd > 0 && dogreensflag == 1) {						//adaptation of diameters (adaptd = 1 or 2)
			...
			adapt();
			printf("\n");
			adaptsignals(imain);		//make plot of signals vs. pressure. Feb. 2017
