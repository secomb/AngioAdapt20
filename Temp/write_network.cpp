...
{
	extern int nseg,nnod,nnodbc,nodsegm,mxx,myy,mzz,nsp,num_runs_actual,seed,dogreensflag;
...
//number of segments in vessel network
	if(dogreensflag == 1) fprintf(ofp,"  %i\t\t\t\ttotal number of segments \n", numseg);
...
//number of nodes in vessel network
	if(dogreensflag ==1) fprintf(ofp," %i       total number of nodes\n", numnod);
...
//boundary nodes
	...
	fprintf(ofp, "\n");
	fclose(ofp);
	//dogreensflag = dogreensflag_temp;
	return;
}