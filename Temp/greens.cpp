...
void greens(void)
{
	...
	extern float* sourcefac;	//added Feb 2020
...
//index tissue points
	for(i=1; i<=mxx; i++) for(j=1; j<=myy; j++) for(k=1; k<=mzz; k++){
		...
		if(nt > 0){
			...
			tisspoints[3][nt] = k;
			sourcefac[nt] = 1.;
			if (k * 5 <= mzz) sourcefac[nt] = 0.;	//this turns off oxygen consumption near the bottom surface
			if ((mzz - k + 1) * 10 <= mzz)
				sourcefac[nt] = 0.;	//this turns off oxygen consumption near the top surface
			//if (5. * (k - 0.5) < mzz) sourcefac[nt] = 5. * (k - 0.5) / mzz;	//this tapers oxygen consumption near the bottom surface
			//if (10. * (mzz - k + 0.5) < mzz) 
			//	sourcefac[nt] = 10. * (mzz - k + 0.5) / mzz;	//this tapers oxygen consumption near the top surface
		}
	}
