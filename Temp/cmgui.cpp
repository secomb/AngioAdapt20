...
void cmgui(int num_runs_actual)
{
	extern int max,mxx,myy,mzz,nseg,nnod,nsegprev,imain,imainmax,hexperiodic;
	...
	if(imain == imainmax){	//cmgui wants a single element file for all time points
		exelem = fopen("Current\\network.exelem","w");
	...
	sprintf(fname,"Current\\network%03i.exnode",num_runs_actual);
	...
	if(num_runs_actual > 0){
		sprintf(fname,"Current\\network%03i.exnode",num_runs_actual-1);
