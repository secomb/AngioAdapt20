...
void input()
{
	extern int nseg,nnod,nnnodbc,nmaxvessel,nmaxtissue,nmax,rungreens,g0method,linmethod,useGPU,imainmax;
	extern int mxx,myy,mzz,nseg,nnod,nsp,nnodbc,nre,nodsegm,nsprmax;
	extern int nitmax1,nitmax,varyviscosity,phaseseparation,eliminate,seed,runtyp,num_runs_actual;
	extern int adaptd,inftrans,angio,adjustks,tensionm,varylb,outbounmethod;
	extern int ncontourplanes,*slsegdiv,*nsl1,*nsl2;
	extern int is2d,angio3d, hexperiodic;
	extern int retinaflag; //for retina
	...
	extern float alx,aly,alz,vol,lb,maxl,req,tumorrad;
	extern float tol,qtol,hdtol,omega,optw,optlam;
	extern float eqtime,diamthresh, kmetdiam;
	extern float smax_flow,smax_noflow,Vmax,min_length,thresh,variance;
	extern float new_diam,increment,tolerance_1,tolerance_2,threshn,thresh50;
	extern float kgGF, kgVAF, kv,lambdatension,tensionrate,r0,theta0,constvisc,consthd;
	extern float kp1,km1,kc1,ks1,ranks1,L1,J01,tauref1,Qref1,pO2ref1,nmeta1,extlength1in,extlength1out,timestep1;
	extern float IR,OR;	//for retinal annulus, mesh guidance
	...
	float totalq, mcv, ahex, khex, dummy;
	int i,inodbc,iseg,isp,max=200,nlmax,dohexflag;
...
//adaptation parameters from Pries et al. 2001
	...
	fscanf(ifp,"%f%*[^\n]", &min_length);
	fscanf(ifp,"%f%*[^\n]", &thresh);
	fscanf(ifp,"%f%*[^\n]", &dummy);	//ignore this line in file
	fscanf(ifp,"%f%*[^\n]", &variance);
	fscanf(ifp,"%f%*[^\n]", &new_diam);
	fscanf(ifp,"%f%*[^\n]", &increment);
	fscanf(ifp,"%f%*[^\n]", &tolerance_1);
	...
	fscanf(ifp,"%f%*[^\n]", &lambdatension);
	fscanf(ifp,"%f%*[^\n]", &tensionrate);
	fscanf(ifp,"%i%*[^\n]", &seed);	//note if runtyp = 2 this is overridden by value in NetworkLast.dat
	fscanf(ifp,"%f%*[^\n]", &r0);
	fscanf(ifp,"%f%*[^\n]", &theta0);
	fscanf(ifp,"%i %f %f%*[^\n]", &dohexflag,&ahex,&khex);
	fscanf(ifp, "%i%*[^\n]", &angio3d);
	...
	fscanf(ifp,"%f%*[^\n]", &alphab);
	fscanf(ifp,"%f%*[^\n]", &q0);
	fscanf(ifp,"%i", &nsp); fgets(bb,max,ifp);
...
//intravascular oxygen resistance data.  Assume zero unless specified in data file.  Multi-solute version, TWS2010.
...
//network data file
	if(runtyp == 1) ifp = fopen("Network.dat", "r");
	else if(runtyp == 2) ifp = fopen("Networklast.dat", "r");
	else printf("*** Error: illegal runtyp\n");
	num_runs_actual = 0;
	time1 = 0.;
	if(runtyp == 2){
		fscanf(ifp, "%i %i %f",&seed,&num_runs_actual,&time1);
		printf("%i %i %f ",seed,num_runs_actual,time1);
	}
	fgets(bb,max,ifp);
	printf("%s\n",bb);
	fscanf(ifp, "%f %f %f%*[^\n]", &alx,&aly,&alz);
	...
	for(iseg=1; iseg<=nseg; iseg++){
		...
		//segtyp[iseg] = 5;
		//hd[iseg] = 0.4;
//Modified May 09 to accept older network files. The %*[^\n] operator tells fscanf to jump to the next line.
//Used to skip extraneous columns in older files.  If not using an older file, the above two lines can be commented out.
	}
...
//boundary nodes
	...
	fclose(ifp);
//scale flows to total relative inflow rate q0 - disabled for angioadapt
//	for(iseg=1; iseg<=nseg; iseg++) q[iseg] = q[iseg]*q0/totalq;
//	for(inodbc=1; inodbc<=nnodbc; inodbc++) if(bctyp[inodbc] == 2) bcprfl[inodbc] = bcprfl[inodbc]*q0/totalq;
