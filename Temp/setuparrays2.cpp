...
{
	extern int nsp,nnod,nnv_dim,nnt_dim,nnt_old_dim,actnum_dim,nsprmax;
	extern int *indx,*mainseg,*activenode,**tisspoints;
	extern float *rhs,*qvtemp,*dtmin, *sourcefac;
	extern float **qv,**pv,**pev,**pvt,**pvprev,**qvprev,**cv,**dcdp,**cv0,**conv0,**gvv;
	extern float **qt,**qt_old,**pt,**pt_old,**ptprev,**ptv,**qcoeff1,**ax,**al;
	extern double **rhsg,**mat;
	extern double *rhsl,*matx;

//used for angio
	extern int nseg;
	extern int *new_segnum,*new_segtyp,*new_active_segs;
	extern float **activegr; 

	FILE *ofp2;
	ofp2 = fopen("ArrayChanges.out","a");

	nnv_dim = nnv;	//these are the actual dimensions of the arrays. TWS2010
	nnt_dim = nnt;
	nnt_old_dim = nnt;
	actnum_dim = actnum;
...
//used for angio
	...
	int actnum_temp = IMAX(actnum,1);	//TWS2010
