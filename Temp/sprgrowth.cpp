...
void sprgrowth()
{
	...
	extern int **nodseg;
	extern float thresh,threshn,thresh50,req,smax_flow,smax_noflow;
	...
	float rnum,smax,prob,lfac,thresh_func;
	...
	for(iseg=1; iseg<=nseg0; iseg++) if(segtyp[iseg] <= 1 || segtyp[iseg] == 5){
		...
		if(dogreensflag == 0) GF = 1.25*thresh;
		else evalGF(2, req, x, lambdaGF2, cfacGF2, ssGF2, bbGF2, ccGF2, &GF, &GFx, &GFy, &GFz);
		if(GF <= thresh) goto nosprout;
		thresh_func = pow((GF - thresh),threshn)/(pow(thresh50,threshn) + pow((GF - thresh), threshn));
		if(segtyp[iseg] == 5) smax = smax_flow;
		else smax = smax_noflow;
